...
- (void)uploadFileWithUrlString:(NSString*)urlString serverName:(NSString*)serverName filePath:(NSString*)filePath completionHandler:(void (^)(NSURLResponse*, NSData*, NSError*))completionHandler{
    ...
    //设置请求头
    [requestM setValue:@"multipart/form-data; boundary=ithm" forHTTPHeaderField:@"Content-Type"];
    //设置请求体
    requestM.HTTPBody = [self  getfromDataWithServerName:serverName filePath:filePath];
...
-(NSData*)getfromDataWithServerName:(NSString*)serverName filePath:(NSString*)filePath
{
    ...
    //拼接文件类型 MOV
    [stringM appendString:@"Content-Type: image/jpeg/MOV\r\n"];
