...
//    CGSize LabelSize = [message boundingRectWithSize:CGSizeMake(290, 9000) options: NSStringDrawingTruncatesLastVisibleLine | NSStringDrawingUsesLineFragmentOrigin | NSStringDrawingUsesFontLeading attributes:attribute context:nil].size;
    ...
    showview.frame = CGRectMake((kScreenWidth - LabelSize.width - 20)/2, kScreenWidth - 100, LabelSize.width+20, LabelSize.height+10);
...
+ (BOOL) validateMobile:(NSString *)mobile
{
    ...
    NSString *phoneRegex = @"(13\\d|14[579]|15[^4\\D]|16[6]|17[^49\\D]|18\\d|19[89])\\d{8}";
...
//正整数
+ (BOOL) validatePositiveInteger:(NSString *)integerStr
{
    NSString *nicknameRegex = @"^[1-9]\\d*$";
    NSPredicate *passWordPredicate = [NSPredicate predicateWithFormat:@"SELF MATCHES %@",nicknameRegex];
    return [passWordPredicate evaluateWithObject:integerStr];
}
