...
-(void)setIsOriginal:(BOOL)isOriginal{
    ...
    if (_isOriginal==YES) {
        ...
        self.newFrame=CGRectMake(0, (LYHEIGHT-LYWIDTH*0.5)/2, LYWIDTH, LYWIDTH*0.5);
    }
...
- (void)initView
{
    ...
    if (self.isOriginal==YES) {
       ...
       oriWidth = self.newFrame.size.width;
       oriHeight = self.originalImage.size.height * (oriWidth / self.originalImage.size.width);
       oriX = self.newFrame.origin.x + (self.newFrame.size.width - oriWidth) / 2;
       oriY = self.newFrame.origin.y + (self.newFrame.size.height - oriHeight) / 2;
