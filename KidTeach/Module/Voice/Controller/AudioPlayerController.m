...
- (void)setImageWith:(MusicModel *)model{
    ...
    self.underImageView.image = [UIImage imageNamed:@"bg"];
    [self.rotatingView.imageView sd_setImageWithURL:[NSURL URLWithString:model.coverurl] placeholderImage:[UIImage imageNamed:@"PlayerHeader"] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
        if (image) {
...
- (void)updateUIDataWith:(MusicModel *)model{
    self.titleLabel.text = model.audio_name;
    self.singerLabel.text = model.audio_name;
