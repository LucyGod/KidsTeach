...
- (void)drawRect:(CGRect)rect {
    ...
    if(ceil(self.currentVisibleRange.y) <= ([DataCenter shareCenter].klineModelArray.count - 1) && self.currentVisibleRange.x > 0) {
        ...
        while (drawCenterX > 0 && drawIndex > 0 && timeGapCount > 0) {
...
- (void)p_drawText:(NSString *)text content:(CGContextRef)ctx textRect:(CGRect)rect {
    ...
    NSDictionary *attributes = @{
                                 ...
                                 NSForegroundColorAttributeName:ASOColorNormalText,   // 字体颜色
...
- (void)p_initialize {
    ...
    self.backgroundColor = RGBCOLOR(25, 25, 29);
...
- (void)klineView:(KLineView *)view didMoveToPoint:(CGPoint)point selectedItemIndex:(NSInteger)index {
    ...
    NSString *dateString = [NSString gl_convertTimeStamp:(tempModel.stamp / 1000) toFormatter:@"yyyy-MM-dd HH:mm"];
...
- (void)p_showDetailViewWithKLineModel:(KLineModel *)model isLeft:(BOOL)isLeft {
    ...
    // 时间
    DetailDataModel *timeModel = [[DetailDataModel alloc] initWithName:@"日期" desc:[NSString gl_convertTimeStamp:(model.stamp / 1000) toFormatter:@"yyyy-MM-dd hh:mm"]];
    // 开
    DetailDataModel *openModel = [[DetailDataModel alloc] initWithName:@"开盘" desc:[@(model.open) gl_numberToStringWithDecimalsLimit:self.kLineMainView.dataCenter.decimalsLimit]];
    // 高
    DetailDataModel *highModel = [[DetailDataModel alloc] initWithName:@"最高" desc:[@(model.high) gl_numberToStringWithDecimalsLimit:self.kLineMainView.dataCenter.decimalsLimit]];
    
    // 低
    DetailDataModel *lowModel = [[DetailDataModel alloc] initWithName:@"最低" desc:[@(model.low) gl_numberToStringWithDecimalsLimit:self.kLineMainView.dataCenter.decimalsLimit]];
    
    // 收
    DetailDataModel *closeModel = [[DetailDataModel alloc] initWithName:@"收盘" desc:[@(model.close) gl_numberToStringWithDecimalsLimit:self.kLineMainView.dataCenter.decimalsLimit]];
    
    // 量
    DetailDataModel *volModel = [[DetailDataModel alloc] initWithName:@"总量" desc:[@(model.volume) stringValue]];
...
- (KLineView *)kLineMainView {
    if (!_kLineMainView) {
        ...
        _kLineMainView.backgroundColor = ASOColorKlineBG;
        // 添加绘图算法
...
- (KLineView *)volView {
    if (!_volView) {
        ...
        _volView.backgroundColor = ASOColorKlineBG;
        // 替换逻辑处理对象为主图逻辑处理对象
...
- (UIView *)verticalLineView {
    if (!_verticalLineView) {
        ...
        [_verticalLineView setBackgroundColor:ASOColorTipText];
...
- (UIView *)horizontalLineView {
    if (!_horizontalLineView) {
        ...
        _horizontalLineView.backgroundColor = ASOColorTipText;
        _horizontalLineView.userInteractionEnabled = NO;
    }
