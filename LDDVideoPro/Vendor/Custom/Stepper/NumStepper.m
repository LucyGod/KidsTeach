...
- (void)drawRect:(CGRect)rect {
    ...
    [NSObject gl_drawTextBackGroundInRect:rect content:ctx boderColor:ASOColorBorder boderWidth:SCALE_Length(1.0f) fillColor:[UIColor clearColor]];
}
...
- (void)setIsCanEdit:(BOOL)isCanEdit {
    ...
    if (_isCanEdit) {
        self.textField.backgroundColor = ASOColorBackGround;
    }else {
        self.textField.backgroundColor = ASOColorSeparator;
    }
...
- (IndentTextField *)textField {
    if (!_textField) {
        ...
        _textField.backgroundColor = ASOColorBackGround;
